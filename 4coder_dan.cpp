...
inline bool
IsLUA(String extension)
{
    bool Result = (match(extension, make_lit_string("lua")) != 0);
...
inline bool
IsPY(String extension)
{
    bool Result = (match(extension, make_lit_string("py")) != 0);
...
inline bool
IsSH(String extension)
{
    bool Result = (match(extension, make_lit_string("sh")) != 0);
...
{
    bool Result = (IsJavascript(extension) || IsBee(extension) || IsH(extension) || IsCPP(extension) || IsINL(extension) || IsBAT(extension) || IsCMirror(extension) || IsShader(extension) || IsMTD(extension) || IsLUA(extension) || IsPY(extension) || IsSH(extension));
...
CUSTOM_COMMAND_SIG(dan_create_view)
{
    View_Summary old_view = get_active_view(app, AccessProtected);
    open_view(app, &old_view, ViewSplit_Right);
    View_Summary new_view = get_active_view(app, AccessProtected);
    view_set_setting(app, &new_view, ViewSetting_ShowScrollbar, 0);
}

CUSTOM_COMMAND_SIG(dan_remove_view)
{
    View_Summary view = get_active_view(app, AccessProtected);
    close_view(app, &view);
}
...
UpdateModalIndicator(Application_Links *app)
{
    ...
    {
        {Stag_Keyword, 0x6b6b6b},
        {Stag_Preproc, 0xf45353},
        {Stag_Default, 0xcecece},
        {Stag_Comment, 0xf45353}
    };
#else      
...
internal void 
mac_custom_keys(Bind_Helper *context)
{
    ...
    bind(context, 'm', MDFR_CMND, build_search);
    bind(context, 'n', MDFR_CMND, goto_next_error);
    bind(context, 's', MDFR_CTRL, search);    
    bind(context, 'p', MDFR_CMND, query_replace);
    bind(context, 'w', MDFR_CMND, change_active_panel);
    bind(context, 'c', MDFR_CMND, open_matching_file_cpp);
    bind(context, 'S', MDFR_CMND, save_all_dirty_buffers);

    bind(context, 'i', MDFR_CMND, dan_create_view);
    bind(context, 'o', MDFR_CMND, dan_remove_view);
    ...
    bind(context, 'x', MDFR_CTRL, auto_tab_range);
    ...
    bind(context, 'q', MDFR_CTRL, copy);
    bind(context, 'f', MDFR_CTRL, paste);
    bind(context, 'w', MDFR_CTRL, delete_range);
    bind(context, 'u', MDFR_CMND, undo);
    bind(context, '\t', MDFR_NONE, write_character);
    bind(context, 'c', MDFR_CTRL, word_complete);
    bind(context, '\t', MDFR_ALT, cursor_mark_swap);
			    
    
    bind(context, key_down, MDFR_ALT, modal_page_down);
    bind(context, key_up, MDFR_ALT, modal_page_up);
    bind(context, key_up, MDFR_ALT, modal_page_up);

    
    bind(context, key_right, MDFR_ALT, seek_white_or_token_right);
    bind(context, key_left, MDFR_ALT, seek_white_or_token_left);
    
    
    bind(context, key_left, MDFR_CMND, casey_seek_beginning_of_line);
    bind(context, key_right, MDFR_CMND, seek_end_of_line);
    bind(context, key_up, MDFR_CMND, move_up_10);
    bind(context, key_down, MDFR_CMND, move_down_10);

    bind(context, key_del, MDFR_ALT, delete_word);
    bind(context, key_back, MDFR_ALT, backspace_word);

    bind(context, key_page_up, MDFR_NONE, move_up_10);
    bind(context, key_page_down, MDFR_NONE, move_down_10);
